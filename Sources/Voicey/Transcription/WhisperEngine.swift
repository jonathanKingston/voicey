...
final class WhisperEngine {
    ...
    func preloadModel() async {
        ...
        debugPrint("📋 Downloaded models: \(ModelManager.shared.downloadedModels.map { $0.rawValue })", category: "MODEL")
    ...
    func loadModel(variant: String = "base.en") async throws {
        ...
        debugPrint("⏳ Loading model '\(variant)' (first run may take 1-3 minutes for CoreML compilation)...", category: "MODEL")
    ...
    func transcribe(audioBuffer: [Float]) async throws -> TranscriptionResult {
        ...
        AppLogger.transcription.info("WhisperEngine: Calling whisperKit.transcribe() with \(audioBuffer.count) samples...")
        ...
        AppLogger.transcription.info("WhisperEngine: Segments: \(result.segments.count), Language: \(result.language)")
