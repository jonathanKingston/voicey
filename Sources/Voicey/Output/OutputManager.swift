...
final class OutputManager {
    ...
    func saveFrontmostApp() {
        ...
        if let menuApp = workspace.menuBarOwningApplication,
           menuApp.bundleIdentifier != myBundleId {
            previousApp = menuApp
            AppLogger.output.info("SaveApp: Saved menu bar app: \(menuApp.localizedName ?? "unknown") (bundle: \(menuApp.bundleIdentifier ?? "?"))")
            return
        }
        ...
        if let frontmost = workspace.frontmostApplication,
           frontmost.bundleIdentifier != myBundleId {
            ...
            AppLogger.output.info("SaveApp: Saved frontmost app: \(frontmost.localizedName ?? "unknown") (bundle: \(frontmost.bundleIdentifier ?? "?"))")
            return
        }
        ...
        for app in workspace.runningApplications {
            if app.isActive && app.bundleIdentifier != myBundleId {
                previousApp = app
                AppLogger.output.info("SaveApp: Saved active app: \(app.localizedName ?? "unknown") (bundle: \(app.bundleIdentifier ?? "?"))")
                return
            }
        }
    ...
    func deliver(text: String, completion: (() -> Void)? = nil) {
        ...
        if let clipboardText = clipboardManager.currentText() {
            if clipboardText == text {
                ...
                AppLogger.output.warning("Deliver: Clipboard mismatch! Expected \(text.count) chars, got \(clipboardText.count)")
            }
        } else {
        ...
        if mode == .pasteOnly || mode == .both {
            ...
            if let app = previousApp {
                AppLogger.output.info("Deliver: Will activate '\(app.localizedName ?? "unknown")' (bundle: \(app.bundleIdentifier ?? "?"), pid: \(app.processIdentifier))")
                AppLogger.output.info("Deliver: App state - isActive: \(app.isActive), isTerminated: \(app.isTerminated), isHidden: \(app.isHidden)")
                ...
                Task { @MainActor in
                    ...
                    if activated {
                        ...
                        AppLogger.output.warning("Deliver: App activation timed out, attempting paste anyway...")
                    }
    ...
    private func activateAppAndWait(_ app: NSRunningApplication, timeout: TimeInterval) async -> Bool {
        AppLogger.output.info("Activate: Sending activate request to '\(app.localizedName ?? "unknown")'")
        ...
        while Date() < deadline {
            pollCount += 1
            if app.isActive {
                AppLogger.output.info("Activate: App became active after \(pollCount) polls (~\(pollCount * 30)ms)")
                ...
                AppLogger.output.info("Activate: After settle, frontmost is: '\(currentFront?.localizedName ?? "none")'")
        ...
        let currentFront = NSWorkspace.shared.frontmostApplication
        AppLogger.output.warning("Activate: Timeout after \(pollCount) polls. Current frontmost: '\(currentFront?.localizedName ?? "none")'")
        return false
    }
}
