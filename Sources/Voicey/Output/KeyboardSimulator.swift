...
final class KeyboardSimulator {
  ...
  func simulatePaste() {
    ...
    AppLogger.output.info(
      "Paste: Current frontmost app: \(frontApp?.localizedName ?? "none") (bundle: \(frontApp?.bundleIdentifier ?? "?"))"
    )
  ...
  private func simulatePasteViaCGEvent() {
    ...
    guard let source = CGEventSource(stateID: .combinedSessionState) else {
      AppLogger.output.error(
        "Paste: Failed to create CGEventSource - this usually means accessibility permissions are missing"
      )
      return
    }
