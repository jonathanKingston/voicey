...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private func setupEscapeKeyMonitor() {
            ...
            if event.keyCode == UInt16(kVK_Escape) {
                Task { @MainActor in
                    ...
                    if self?.appState.transcriptionState.isActive == true {
                        AppLogger.general.info("ESC pressed - cancelling (state: \(String(describing: self?.appState.transcriptionState)))")
        ...
        localEscKeyMonitor = NSEvent.addLocalMonitorForEvents(matching: .keyDown) { [weak self] event in
            if event.keyCode == UInt16(kVK_Escape) {
                // Cancel if in any active state (loading, recording, or processing)
                if self?.appState.transcriptionState.isActive == true {
                    AppLogger.general.info("ESC pressed (local) - cancelling (state: \(String(describing: self?.appState.transcriptionState)))")
                    self?.cancelTranscription()
                    return nil
                }
            }
    ...
    private func checkPermissionsSilently() async {
        ...
        AppLogger.general.info("Permission status - Microphone: \(micPermission), Accessibility: \(accessibilityPermission)")
    ...
    func startRecording() {
        ...
        AppLogger.general.info("startRecording: Downloaded models: \(downloadedModels.map { $0.rawValue })")
        AppLogger.general.info("startRecording: Is selected model downloaded? \(ModelManager.shared.isDownloaded(selectedModel))")
        ...
        if !ModelManager.shared.isDownloaded(selectedModel), let firstDownloaded = downloadedModels.first {
            AppLogger.general.info("startRecording: Selected model not available, switching to \(firstDownloaded.rawValue)")
            ...
            Task {
                ...
                await MainActor.run {
                    if appState.modelStatus.isReady {
                        ...
                        self.dependencies.notifications.showTranscriptionError("Model failed to load. Please try again.")
                    }
                }
            }
            return
    ...
    func stopRecording() {
        ...
        debugPrint("📊 Got \(audioBuffer.count) samples (~\(String(format: "%.1f", durationSec))s of audio)", category: "AUDIO")
        AppLogger.audio.info("Got audio buffer with \(audioBuffer.count) samples (~\(String(format: "%.1f", durationSec))s)")
        
        // Check minimum duration (0.5 seconds)
        if durationSec < 0.5 {
            debugPrint("⚠️ Audio too short (\(String(format: "%.2f", durationSec))s), skipping", category: "AUDIO")
            AppLogger.audio.warning("Audio too short (\(String(format: "%.2f", durationSec))s), skipping transcription")
            hideOverlay()
    ...
    private func processTranscription(audioBuffer: [Float]) async {
        do {
            ...
            AppLogger.transcription.info("processTranscription: Starting with \(audioBuffer.count) samples")
            ...
            AppLogger.transcription.info("processTranscription: Processed text: \"\(processedText)\" (length: \(processedText.count))")
            ...
            await MainActor.run {
                ...
                if processedText.isEmpty {
                    ...
                    AppLogger.transcription.warning("processTranscription: No text to deliver (empty after processing)")
                    self.hideOverlay()
