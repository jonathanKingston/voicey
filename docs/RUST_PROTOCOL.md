...
- **Text postprocess (#63):** `cargo test -p voicey-text` — noise filter, voice commands, JSONL worker loop; `cargo test -p voicey-text --test text_ipc` — black-box worker binary (Tier 1)
